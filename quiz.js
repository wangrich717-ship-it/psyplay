...
const QUIZZES = [
  ...
  // ── TEST 1: 压力颜色 ────────────────────────────────────────────────────────
  {
    ...
    questions: [
      ...
      },
      {
        ...
        options: [
          { hex: '#C8A83A', name: '暗金黄', cat: 'S' },
          { hex: '#8B0000', name: '血红', cat: 'S' },
          { hex: '#5C9B4A', name: '草木绿', cat: 'S' },
          { hex: '#9B8FA3', name: '灰紫', cat: 'G' },
          { hex: '#3D2B1F', name: '暗棕', cat: 'G' },
          { hex: '#B8D8EA', name: '冰蓝', cat: 'C' },
          { hex: '#CC789A', name: '藕粉', cat: 'S' },
          { hex: '#C04A2A', name: '砖红', cat: 'S' },
  ...
  // ── TEST 3: 情绪音调 ────────────────────────────────────────────────────────
  {
    ...
    results: {
      sensitive: {
        title: '你的情绪像「绷紧的高弦」',
        accent: '#c4557c',
        p: ['你对外界刺激的感知极其细腻。声音、语气、空气里细微的变化，你都能第一时间捕捉到。', '高弦的音调清澈，共鸣强——这是你的天赋，也是消耗。你感受得比别人更深，也比别人更容易疲惫。', '学会给自己设置"调弦间歇"——不是关闭感受，而是偶尔放松弦的张力，让它休息一下。'],
        quote: '高弦最容易出声，也最容易断。调好张力，才能弹出最美的曲。'
      },
      balanced: {
        title: '你的情绪像「校准好的调音叉」',
        accent: '#4aa8c8',
        p: ['你有稳定的内在基准音，不容易被外界的杂声带乱节奏。', '你知道什么时候该共鸣，什么时候该保持沉默——这是一种难得的心理弹性。', '继续保持这种校准，同时偶尔允许自己被一些事情"搅动"——那也是活着的证明。'],
        quote: '好的调音叉，既能对准一个频率，也能静静等待下一次敲击。'
      },
      mixed: {
        title: '你的情绪像「叠加的泛音」',
        accent: '#9b7ec8',
        p: ['你的感受常常是多层叠加的——同一件事让你同时感到开心和难过，同时感到渴望和抗拒。', '这不是问题。泛音越丰富，音色越有层次感。你拥有复杂而立体的情绪频谱，这是一种天赋。', '试着不急于分辨"我到底是什么感觉"，允许几种频率同时共鸣。'],
        quote: '最丰富的音色，从来不是单一频率的——它是无数泛音同时存在的产物。'
      },
      defensive: {
        title: '你的情绪像「按下了静音键」',
        accent: '#7ba7c0',
        p: ['你已经学会了自动屏蔽一部分感受。不是你冷漠，是你曾经太响了，所以后来选择了静音。', '你的内在系统在保护你——通过降低音量来减少消耗。这是一种自救，但也有代价。', '偶尔把静音键抬起来，哪怕只是几分钟，试着听听周围真实的声音——哪怕它有些嘈杂。'],
        quote: '静音是为了保护耳朵，但不要让这个世界的声音永远到不了你那里。'
      },
    }
  },
  ...
  // ── TEST 7: 你用什么方式爱自己 ──────────────────────────────────────────────
  {
    ...
    calculate(values) {
      ...
      // Group by dimension (2 each), max per dim = 6, total max = 30
      const dims = ['身体照料', '情绪允许', '边界设定', '自我对话', '快乐供给'];
      const scores = dims.map((_, i) => values[i * 2] + values[i * 2 + 1]);
      const total = scores.reduce((a, b) => a + b, 0);
      const key = total >= 22 ? 'high' : total >= 11 ? 'medium' : 'low';
      return { type: key, scores, dims };
    },
    results: {
      high: {
        title: '你已经在好好爱自己了',
        accent: '#c4557c',
        p: ['你在大多数维度上都给了自己相当充分的照顾——身体的感受、情绪的空间、自我对话的温度，你有意识地在维护这些。', '这不是理所当然的事。很多人终其一生都不知道如何照顾自己的内在，而你已经在练习了。', '继续保持，同时也留意那些得分偏低的区域——那里可能藏着你还没注意到的疲惫。'],
        quote: '爱自己不是终点，而是一种持续的选择。你已经选了很多次了。'
      },
      medium: {
        title: '你在学习照顾自己的路上',
        accent: '#c4557c',
        p: ['你有一些维度照顾得不错，也有一些还没被充分关注——这很正常，自我照顾是一门需要时间学习的功课。', '得分偏低的地方不代表你做错了什么，只是那些需求还没有被你看见，或者还没找到合适的方式去满足。', '不需要全面提升，从最低分的那一项开始，做一个小小的调整，就是很好的开始。'],
        quote: '不必在所有地方都做到满分。先照顾最需要你的那个部分。'
      },
      low: {
        title: '你一直在给别人，却忘了给自己',
        accent: '#c4557c',
        p: ['你的照料地图整体偏低——这通常不是因为你不知道如何照顾人，而恰恰相反，你太擅长照顾别人了，却很少把同样的善意留给自己。', '你值得被好好对待。不仅仅被别人，也被你自己。', '从最简单的事开始：今天吃一顿你真正想吃的饭，睡一个你允许自己好好睡的觉——这就是照料的起点。'],
        quote: '你一直知道怎么对别人好。现在，试着用同样的方式对自己好一次。'
      }
    }
  },
